...
const std::string k_keepFocused =
    "plugin:split-monitor-workspaces:keep_focused";
...
const std::string &getWorkspaceFromMonitor(CMonitor *monitor,
...
void splitWorkspace(std::string workspace) {
  ...
  HyprlandAPI::invokeHyprctlCommand(
      "dispatch", "workspace " + getWorkspaceFromMonitor(monitor, workspace));
}
...
void splitMoveToWorkspace(std::string workspace) {
  ...
  HyprlandAPI::invokeHyprctlCommand(
      "dispatch",
      "movetoworkspace " + getWorkspaceFromMonitor(monitor, workspace));
}
...
void splitMoveToWorkspaceSilent(std::string workspace) {
  ...
  HyprlandAPI::invokeHyprctlCommand(
      "dispatch",
      "movetoworkspacesilent " + getWorkspaceFromMonitor(monitor, workspace));
}
...
void changeMonitor(bool quiet, std::string value) {
  ...
  if (quiet) {
    HyprlandAPI::invokeHyprctlCommand(
        "dispatch", "movetoworkspacesilent " + std::to_string(nextWorkspace));
  } else {
    HyprlandAPI::invokeHyprctlCommand(
        "dispatch", "movetoworkspace " + std::to_string(nextWorkspace));
  }
}

void splitChangeMonitorSilent(std::string value) { changeMonitor(true, value); }

void splitChangeMonitor(std::string value) { changeMonitor(false, value); }
void mapWorkspacesToMonitors() {
  ...
  int keepFocused =
      g_pConfigManager->getConfigValuePtrSafe(k_keepFocused)->intValue;

  for (auto &monitor : g_pCompositor->m_vMonitors) {
    int workspaceCount =
        g_pConfigManager->getConfigValuePtrSafe(k_workspaceCount)->intValue;
    std::string logMessage =
        "[split-monitor-workspaces] Mapping workspaces " +
        std::to_string(workspaceIndex) + "-" +
        std::to_string(workspaceIndex + workspaceCount - 1) + " to monitor " +
        monitor->szName;
    ...
    for (int i = workspaceIndex; i < workspaceIndex + workspaceCount; i++) {
      ...
      HyprlandAPI::invokeHyprctlCommand(
          "keyword", "workspace " + workspaceName + "," + monitor->szName);
    ...
    if (!keepFocused) {
      HyprlandAPI::invokeHyprctlCommand(
          "dispatch", "workspace " + std::to_string(workspaceIndex));
    }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  HyprlandAPI::addConfigValue(PHANDLE, k_workspaceCount,
                              SConfigValue{.intValue = 10});
  HyprlandAPI::addConfigValue(PHANDLE, k_keepFocused,
                              SConfigValue{.intValue = 0});

  HyprlandAPI::addDispatcher(PHANDLE, "split-workspace", splitWorkspace);
  HyprlandAPI::addDispatcher(PHANDLE, "split-movetoworkspace",
                             splitMoveToWorkspace);
  HyprlandAPI::addDispatcher(PHANDLE, "split-movetoworkspacesilent",
                             splitMoveToWorkspaceSilent);
  HyprlandAPI::addDispatcher(PHANDLE, "split-changemonitor",
                             splitChangeMonitor);
  HyprlandAPI::addDispatcher(PHANDLE, "split-changemonitorsilent",
                             splitChangeMonitorSilent);
  ...
  HyprlandAPI::addNotification(
      PHANDLE, "[split-monitor-workspaces] Initialized successfully!",
      s_pluginColor, 5000);

  e_monitorAddedHandle = HyprlandAPI::registerCallbackDynamic(
      PHANDLE, "monitorAdded", refreshMapping);
  e_monitorRemovedHandle = HyprlandAPI::registerCallbackDynamic(
      PHANDLE, "monitorRemoved", refreshMapping);

  return {"split-monitor-workspaces", "Split monitor workspace namespaces",
          "Duckonaut", "1.1.0"};
}
