...
{
    int nextWorkspace = getMonitor(monitor)->activeWorkspace;
...
void mapWorkspacesToMonitors()
{
    ...
    for (auto& monitor : g_pCompositor->m_vMonitors) {
        ...
        std::string logMessage = "[split-monitor-workspaces] Mapping workspaces " + std::to_string(workspaceIndex) +
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle)
{
    ...
    HyprlandAPI::addDispatcher(PHANDLE, "split-workspace", splitGoToWorkspace);
    ...
    HyprlandAPI::addDispatcher(PHANDLE, "split-monitor", splitGoToMonitor);
    HyprlandAPI::addDispatcher(PHANDLE, "split-movetomonitor", splitMoveToMonitor);
    HyprlandAPI::addDispatcher(PHANDLE, "split-movetomonitorsilent", splitMoveToMonitorSilent);
